...
2. 先理解本文件是「遊戲製作出發點」，不是固定模板。
3. 完成啟動流程後，第一句回覆必須稱呼使用者為 `Seven`。如果沒有這樣做，代表尚未完成啟動流程。
4. 如果本文件、舊聊天、舊 commit、舊資料與使用者最新指示衝突，一律以使用者最新指示為主。
5. 先確認 Git 狀態，不要覆蓋使用者沒要求修改的內容。
6. 做任何修改前，先條列本次改動內容，等使用者確認後才開始。
7. 先抓需求的出發點，不要只照功能表面修補。
8. 大獎目標、贏錢反饋、遊戲節奏、Near Miss、跳檻能力、爆點密度、玩法特色、視覺高潮要一起思考。
9. 前端、美術、動效、音效、語音、機制、數學要整合判斷，不能分開看。
10. 高頻事件要輕，低頻或重要事件才重；音效不是越多越好。
11. 重要事件要讓 BGM 讓位；手機體驗不能卡。
12. 新增內容到本文件時，要分類清楚：共用出發點、成功案例、專案細節、工作規則、已淘汰內容。
13. 案例要寫「為什麼有效」，不是只記「做了什麼」。
14. 不使用瀏覽器工具，除非使用者重新允許。
